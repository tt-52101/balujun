...
import {ScenicSpotServiceProxy, PagedResultDtoOfScenicSpotListDto, ScenicSpotListDto } from '@shared/service-proxies/service-proxies';
...
export class  ScenicSpotComponent extends PagedListingComponentBase<ScenicSpotListDto>
	implements OnInit {
  ...
  curmenupower=[]
	...
	*/
	delete(entity: ScenicSpotListDto): void {
		...
		abp.message.confirm(
			...
			res => {
				if (res) {
					...
					this._scenicSpotService.batchDelete(ids).subscribe(() => {
						this.refreshGoFirstPage();
						this.notify.success(this.l('SuccessfullyDeleted'));
					});
				}
